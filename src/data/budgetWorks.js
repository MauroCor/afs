export const budgetWorksByCategory = {
  GAS: [
    'FACTIBILIDAD + PARCIAL VIVIENDA UNIFAMILIAR (OBRA NUEVA CASA TIPO)',
    'FINAL (CON INSTALACIÓN DE COCINA) + INSPECCIÓN',
    'PEDIDO DE SOLDADURA/CORTE (TASAS, CARGOS Y PERMISOS DE ECOGAS A CARGO DEL PROPIETARIO)',
    'RETIRO DE MEDIDOR EN JESÚS MARÍA E INSTALACIÓN DEL MISMO Y ADECUACIÓN DE GLP A GAS NATURAL',
    'ADECUACIÓN DE ARTEFACTOS',
    'REJILLAS DE VENTILACIÓN INFERIOR/SUPERIOR',
    'CAÑOS DE VENTILACIÓN DE ARTEFACTOS REGLAMENTARIOS',
    'INSTALACIÓN DE COCINA + 2 PLACAS AISLANTES Y FIJACIÓN',
    'INSTALACIÓN DE CALEFACTOR I.R.',
    'INSTALACIÓN DE CALEFACTOR T.B.',
    'INSTALACIÓN DE CALEFACTOR T.B.U.',
    'INSTALACIÓN DE CALEFÓN T.N.',
    'INSTALACIÓN DE TERMOTANQUE',
    'INSTALACIÓN DE CAÑERÍA DE GAS',
    'OTROS'
  ],
  'AGUA PLUVIAL': [
    'Relevamiento y colocación de canaletas',
    'Instalación de bajadas pluviales'
  ],
  SANITARIO: [
    'Instalación de artefactos sanitarios',
    'Reparación de cañerías sanitarias'
  ],
  CALEFACCION: [
    'Instalación de radiadores',
    'Mantenimiento de caldera'
  ]
};


